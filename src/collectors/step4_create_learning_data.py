...
class Step4Pipeline(DataPipeline):
    ...
    def execute(self) -> bool:
        ...
        print(f"  Spotifyデータ: {len(df_spotify)}行, {df_spotify['artist'].nunique()}アーティスト")
        print(f"  紅白データ: {len(df_kouhaku)}行, {df_kouhaku['artist'].nunique()}アーティスト")
        ...
        try:
            ...
            name_mapping = dict(zip(df_mapping['kouhaku_name'], df_mapping['spotify_name']))
        ...
        spotify_cols = ['artist_normalized', 'year', 'weeks_on_chart', 'total_streams',
                        'best_rank', 'avg_rank', 'top10_weeks', 'top1_weeks']
        df_spotify_subset = df_spotify[spotify_cols].copy()

        # LEFT JOIN
        df_merged = df_base.merge(df_spotify_subset, on=['artist_normalized', 'year'], how='left')
        ...
        df_merged['appeared'] = df_merged.apply(
            lambda row: 1 if (row['artist_normalized'], row['year']) in kouhaku_set else 0,
            axis=1
        )
        ...
        # 累積出場回数（その年より前）
        df_merged = df_merged.groupby('artist_normalized', group_keys=False).apply(calc_past_appearances)

        # 前年出場有無
        df_merged = df_merged.groupby('artist_normalized', group_keys=False).apply(calc_prev_year_appeared)

        # 連続出場年数
        df_merged = df_merged.groupby('artist_normalized', group_keys=False).apply(calc_consecutive_years)

        # ========== [8] 最終的なカラム整理 ==========
        print("\n[8] カラム整理")

        # アーティスト名（元の名前を復元）
        artist_name_map = dict(zip(df_spotify['artist_normalized'], df_spotify['artist']))
        df_merged['artist'] = df_merged['artist_normalized'].map(artist_name_map).fillna(df_merged['artist_normalized'])
