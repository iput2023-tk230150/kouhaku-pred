...
class ArtistMapper:
    ...
    def match_artists(
        ...
        for kouhaku_orig in kouhaku_artists:
            ...
            if best_match:
                ...
                unmatched.append({
                    'kouhaku_name': kouhaku_orig,
                    'variants_tried': ', '.join(kouhaku_variants[:3])  # 最大3つ表示
                })
    ...
    def save_results(
        ...
        if unmatched:
            ...
            df_unmatched.to_csv(
                output_path / 'unmatched_artists.csv',
                index=False,
                encoding='utf-8-sig'
            )
    ...
    def print_summary(
        ...
        if similar_candidates:
            print("\n" + "-" * 70)
            print(f"[要確認] artist_name_mapping_draft.csv ({len(similar_candidates)}件)")
            print("-" * 70)
            df_similar = pd.DataFrame(similar_candidates)
            print(df_similar[['kouhaku_name', 'spotify_name', 'similarity']].to_string(index=False))
        ...
        print(f"  - 自動マッチ成功: {len(matched)} ({100*len(matched)/kouhaku_total:.1f}%)")
        print(f"  - 類似度候補: {len(similar_candidates)} ({100*len(similar_candidates)/kouhaku_total:.1f}%)")
        print(f"  - 未マッチ: {len(unmatched)} ({100*len(unmatched)/kouhaku_total:.1f}%)")
